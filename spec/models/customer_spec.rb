...
RSpec.describe Customer, type: :model do
  ...
  describe 'methods' do 
    ...
    it "knows the top customers" do 
      expected = [@customer_1, @customer_2, @customer_3, @customer_4, @customer_6]

      expect(Customer.top_customers.to_a).to eq(expected)
    end

    it "knows number of transactions" do 
      expect(@customer_1.number_of_transactions).to eq(2)
      expect(@customer_6.number_of_transactions).to eq(1)
    end
  end 
end
